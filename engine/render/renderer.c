...
bool renderer_initialize(const char *app_name, int window_width, int window_height, enum render_backend backend)
{
	...
	case RENDERER_VULKAN:
		...
		renderer.window_flags |= (SDL_WindowFlags)SDL_WINDOW_VULKAN;
		break;
	default:
	...
	// Create a window
	renderer.window = SDL_CreateWindow(app_name,
	...
	if (!renderer.window) {
		FATAL("Couldn't create an SDL window and the application cannot continue.");
		return false;
	}
